...
- The `--observe` trace's `act`-phase span for a committee cycle carries `axiom.router.committee_size` and `axiom.router.providers` (comma-joined provider names) — proving the dispatch attempted every configured member, without requiring a separate span per member (design.md D10 — a single committee-mode span with aggregate attribution, not N single-provider-shaped spans; M6's one-act-span-per-ACT-cycle invariant is preserved, and OR-6's all-fail case keeps failure attribution on that one span rather than losing it once dispatch moves outside any span boundary).
- **[behavioral]** OR-4's live marker-based verification (below) is this story's real per-member-dispatch proof: it only succeeds if every member was genuinely and independently invoked with the instruction, not simulated or relabeled — a synthesized response referencing two DISTINCT providers' own self-descriptions is only reachable if both received and answered the same instruction for real. This story does not require its own separate live CLI run — OR-4's run demonstrates it.

**Revision note (dryrun-code-2 follow-up, found during live-verification prep, 2026-07-27):** the original AC here required "one `act` span per configured provider... each carrying its own `axiom.router.provider` value" — a mechanism the implemented design (D10, dryrun-design-2 PASS) deliberately did not build: one aggregate span with `committee_size`/`providers` attributes instead, specifically to preserve M6's one-span-per-phase invariant and keep OR-6's all-fail case inside a span's failure boundary. Neither dryrun-design pass caught this AC/design conflict (a genuine miss — Pass 1 Completeness and Pass 10 Behavioral DoD should have). Resolved here rather than in code: OR-4's marker test independently and more strongly proves genuine per-member dispatch (dispatch alone, without a real distinct answer reaching synthesis, wouldn't produce two distinguishable provider identities in the final response), so the per-span mechanism was redundant over-specification, not a requirement this story's Purpose actually depends on.
