...
| D10 | **Live-verification finding, fixed at the source:** `RetrievalPipeline.recall()` (M3, `src/axiom/memory/retrieval.py`) only passed `type_filter` to the semantic strategy (`vector_search`) — `fulltext_search` (keyword) and `get_by_recency` (temporal) ran with no type constraint at all, and RRF-fused their untyped hits into the same ranked result set before `limit` truncation. A live two-turn verification run surfaced this directly: `recall(type_filter="lesson")` returned 1 genuine lesson plus 2 unrelated `episodic` entries, and — the worse failure mode — a real lesson can be silently crowded out of the final `limit`-N slots by higher-scoring untyped keyword/temporal hits with **no error, no warning, degrading silently as the memory store grows**. This is pre-existing M3 behavior (SC-2 is the first M8 consumer to depend on `type_filter` actually filtering), root-caused and fixed at the source in `retrieval.py`: `keyword_results`/`temporal_results` are now filtered to `memory_type == type_filter` immediately after the `asyncio.gather(...)` and before RRF fusion, so all three strategies agree on type before scoring/truncation. Rejected the alternative of a defensive filter inside `loop.py`'s own INJECT code — that would suppress the visible symptom (wrong-typed entries in the rendered section) while leaving the invisible one (real lessons silently losing their ranked slot to untyped competitors) completely unfixed. Verified via the same live verification script post-fix: `recall(type_filter="lesson")` now returns exactly the 1 genuine lesson, and the rendered `[LESSONS FROM PAST CORRECTIONS]` section contains only that lesson. |
...
| `src/axiom/memory/retrieval.py` | D10 (live-verification finding): `recall()`'s keyword/temporal strategy results are now re-filtered to `memory_type == type_filter` before RRF fusion, so `type_filter` applies uniformly across all three strategies instead of only the semantic one. | SC-2 |
