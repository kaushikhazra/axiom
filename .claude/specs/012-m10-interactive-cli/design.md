...
| `_CHUNK_DELAY_SECS` | `agui_bridge.py` | `0.02` | Delay between chunked `TEXT_MESSAGE_CONTENT` deltas (D7). |
| `_CANVAS_LINE_THRESHOLD` | `canvas_routing.py` | `15` | Minimum fenced-code-block line count to route to canvas instead of inline chat (D8). |
...
async def stream_turn(session: WebSession, user_input: str, thread_id: str):
    ...
    for chunk in _chunk_response(chat_text):          # D7 -- chunked, not real token streaming
...
| `src/axiom/interface/web/agui_bridge.py` | New — `stream_turn()`: concurrent turn-task + event-queue drain (D14), canvas emission (D8/D13), chunked delivery (D7). | US-02, US-03, US-06 |
