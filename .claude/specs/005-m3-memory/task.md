...
- [x] 11.3 `loop.py` — loop author calls `await self.memory.append_unit(unit)` at Observe: constructs a `ConversationUnit(user_text=..., agent_text=..., turn_index=..., timestamp=now)` from the completed turn and awaits `append_unit`. `store` is NOT used for working-context writes in `src/axiom/loop.py`. _MEM-US02_
  > **CV1 reconciliation (2026-07-15):** The design lists `store(content, ...)` at Observe as "one call per cognitive knowledge item to persist." However, design §16 explicitly defers LLM-assisted extraction to M8. Without an extraction mechanism, the loop has no way to decide *what* to store — auto-storing every response would write noise. **Decision: cognitive `store` calls at Observe are AGENT-DRIVEN (future M8 callsite), not automatic per turn.** `append_unit` is the only Observe write in M3. This task is marked done for the working-context path; the cognitive-store callsite is an M8 task.
